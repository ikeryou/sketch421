...
export class Item extends MyDisplay {
  ...
  protected _update():void {
    ...
    const rad = this._c * -3 + this._itemId * 3 * (this._itemId % 2 != 0 ? -1 : 1)
    const p1 = 0;
    ...
    this._color3.g = Util.map(Math.sin(Util.radian(this._angle3)), 0, 1, -1, 1)
    ...
    const angle = Math.cos(Util.radian(rad)) * 20
    // const angle = (this._itemId - 11) * 10

    //const col1 = 'rgba(' + (color1.r * 255) + ',' + (color1.g * 255) + ',' + (color1.b * 255) +', 0)'
    //const col2 = 'rgba(' + (color2.r * 255) + ',' + (color2.g * 255) + ',' + (color2.b * 255) +', 0)'


    const grad = 'linear-gradient(' + (angle) + 'deg, ' + color1.getStyle() + ' ' + p1 + '%, ' + color2.getStyle() + ' ' + (p2 - lineWidth) + '%, ' + lineColor.getStyle() + ' ' + p2 + '%, ' + lineColor.getStyle() + ' ' + p2 + '%, ' + color3.getStyle() + ' ' + (p2 + lineWidth) + '%, ' + color2.getStyle() + ' ' + p3 + '%)'
